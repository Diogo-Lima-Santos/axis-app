...
function initTutorial() {
  ...
  for (let i = 1; i <= TOTAL_STEPS; i++) {
    ...
    d.className = 'tut-dot' + (i === 1 ? ' active' : '');
    d.onclick = () => jumpTutStep(i);
...
function jumpTutStep(n) {
  document.getElementById('tstep-' + currentTutStep)?.classList.remove('active');
  currentTutStep = n;
  document.getElementById('tstep-' + currentTutStep)?.classList.add('active');
  renderTutStep();
}

function renderTutStep() {
  const n = currentTutStep;

  // Progress bar
  document.getElementById('tpb-fill').style.width = ((n / TOTAL_STEPS) * 100) + '%';
  ...
  back.classList.toggle('hidden-btn', n === 1);

  // Next button
  const next = document.getElementById('tut-btn-next');
  if (n === TOTAL_STEPS) {
    next.innerHTML = 'Ir para o Dashboard <svg viewBox="0 0 20 20" fill="none"><path d="M8 5l5 5-5 5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    next.classList.add('finish');
  } else {
    next.innerHTML = 'Próximo <svg viewBox="0 0 20 20" fill="none"><path d="M8 5l5 5-5 5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function goTo(screen) {
  ...
  if (screen === 'tutorial') { currentTutStep = 1; initTutorial(); }
}
